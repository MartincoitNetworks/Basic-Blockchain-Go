...
// miner address is the address of the miner
...
//nodes sent each other information
// The structures below detail what information is passing in the peer-to-peer network
type Addr struct {
	AddrList []string
}
...
// to sync blockchains between each of our nodes
type Version struct {
	Version    int
	BestHeight int
	AddrFrom   string
...
//enough tx-> mine block
//block sent to central node
//wallet syncs and verifies
// convert o bytes
...
//iterators through all known nodes and calls SendGetBlocks
func RequestBlocks() {
...
//check the length of blockchain and send
...
//send are the client function i.e messages we are sending to peers
// we need handle functions to handle those requests
//serialise into an address structure
func HandleAddr(request []byte) {
	var buff bytes.Buffer
	var payload Addr
	//get command with write
...
// take data and convert into a slice of bytes
//used to send commands to and from our application
...
//death library- to close database if we hit ctrl  +c
